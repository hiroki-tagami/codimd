...
function showNotes (req, res, next) {
  ...
  models.Note.findAll({
    ...
    data.notes = notes.map(function (note) {
      return {
        noteId: models.Note.encodeNoteId(note.id),
        info: models.Note.parseNoteInfo(note.content),
        fromNow: moment(note.updatedAt).fromNow()
      }
    })
...
function searchNotes (req, res, next) {
  ...
  if (req.query.word != undefined) {
    searchAs.where.content = {[models.Sequelize.Op.like]:'%' + req.query.word + '%'}
  }

  // search as title
  if (req.query.title != undefined) {
    searchAs.where.title = {[models.Sequelize.Op.like]:'%' + req.query.title + '%'}
  }

  searchAs.order = [[ 'updatedAt', 'DESC' ]]

  models.Note.findAll(searchAs)
    .then(function (notes) {
    data.notes = notes.map(function (note) {
      var result = "";
      if(req.query.word != undefined) {
	var results = note.content.split(req.query.word).slice(1)
	for(var r of results) {
	  result += '<strong style="background-color:yellow">' + req.query.word + '</strong>' + r.substr(0, 150) + '....'
	}
      } else {
	result = note.content.substr(0, 250)
      }
